...
export const i18n: Record<string, Dict> = {
  zh: {
    ...
    use_body: '{app} 支持 Ofox 作为模型供应商。Ofox 是统一的 AI 模型网关——一个 API Key 即可调用众多模型，内置负载均衡与自动故障转移,随时自由切换、无需逐家对接。',
  ...
  'zh-TW': {
    ...
    use_title: '使用 Ofox 和 {app}',
    use_body: '{app} 支援 Ofox 作為模型供應商。Ofox 是統一的 AI 模型閘道——一個 API Key 即可呼叫眾多模型，內建負載平衡與自動故障轉移，隨時自由切換、無需逐家對接。',
  ...
  en: {
    ...
    use_body: '{app} supports Ofox as a model provider. Ofox is a unified AI model gateway — one API key to call a wide range of models, with built-in load balancing and automatic failover, so you can switch freely without wiring up each provider.',
  ...
  ja: {
    ...
    use_body: '{app} は Ofox をモデルプロバイダーとしてサポートしています。Ofox は統合 AI モデルゲートウェイで、1 つの API キーで多数のモデルを呼び出せます。負荷分散と自動フェイルオーバーを内蔵し、各社に個別接続することなく自由に切り替えられます。',
  ...
  ko: {
    ...
    use_body: '{app}은(는) Ofox를 모델 공급자로 지원합니다. Ofox는 통합 AI 모델 게이트웨이로, 하나의 API 키로 다양한 모델을 호출할 수 있습니다. 부하 분산과 자동 페일오버를 기본 제공하여 공급자마다 따로 연동할 필요 없이 자유롭게 전환할 수 있습니다.',
  ...
  de: {
    ...
    use_body: '{app} unterstützt Ofox als Modellanbieter. Ofox ist ein einheitliches AI-Modell-Gateway — mit einem einzigen API-Key rufst du zahlreiche Modelle auf, mit integriertem Load-Balancing und automatischem Failover, sodass du frei wechseln kannst, ohne jeden Anbieter einzeln anzubinden.',
  ...
  es: {
    ...
    use_body: '{app} admite Ofox como proveedor de modelos. Ofox es una pasarela unificada de modelos de IA: con una sola API Key llamas a numerosos modelos, con balanceo de carga y conmutación por error integrados, para cambiar libremente sin integrar cada proveedor por separado.',
  ...
  fr: {
    ...
    use_body: '{app} prend en charge Ofox comme fournisseur de modèles. Ofox est une passerelle de modèles d’IA unifiée : une seule clé API permet d’appeler de nombreux modèles, avec répartition de charge et bascule automatique intégrées, pour changer librement sans intégrer chaque fournisseur séparément.',
  ...
  pt: {
    ...
    use_body: '{app} oferece suporte à Ofox como provedor de modelos. A Ofox é um gateway unificado de modelos de IA — com uma única API Key você chama diversos modelos, com balanceamento de carga e failover integrados, para alternar livremente sem integrar cada provedor separadamente.',
  ...
  ru: {
    ...
    use_body: '{app} поддерживает Ofox как поставщика моделей. Ofox — это единый шлюз AI-моделей: один API-ключ позволяет вызывать множество моделей, со встроенной балансировкой нагрузки и автоматическим переключением при сбоях, чтобы свободно менять модели без отдельной интеграции каждого поставщика.',
...
// Hero rewards card ("limited-time call"). Intentionally teasing — $10 is the
